...
> **UPDATE 2026-08-10 (K-BATCH K3): real root cause found, real fix, real re-run — see `docs/RECEIPTS.md`'s "K-BATCH" section for full detail.** The 17% yield below was NOT primarily the extraction-heuristic bug this section originally suspected (Finding 1) — verified on 5 real winner tokens that the actual cause was `_fetch_sigs`'s alert-time-window pagination giving up (capped at 8,000 signatures) before ever reaching a high-activity token's own alert time. Fixed via slot-time estimation. Re-ran the 400: **165/225 newly-processed tokens got real rows (73.3%, target >70% met)**. `path_stats.py` now clears n>=100 on sections C, E(overall), F(overall), G, H — only the progress-bucketed cells (A/B, needs `progress_at_signal` on the same tokens as path data, only reliably available since PROGRESS-FIX) and D (graduation velocity) remain below the bar. The historical Aug 3 diagnosis below is kept for the record but its "STILL BLOCKED" conclusion is superseded.

> **STATUS 2026-08-03 (historical, superseded above): STILL BLOCKED — do not treat as ready for the V8 freeze session.** PC2 backfill ran to completion (400/400 tokens, 0 script errors), but see the two root-cause findings below. Net usable data: **67 path files, all backfill-sourced, 0 forward-collected.** Every cell below needs n≥100; none clear that bar.
